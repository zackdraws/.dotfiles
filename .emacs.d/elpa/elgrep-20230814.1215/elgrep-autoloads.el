;;; elgrep-autoloads.el --- automatically extracted autoloads (do not edit)   -*- lexical-binding: t -*-
...
(autoload 'elgrep-mode "elgrep" "\
Major mode for elgrep buffers.
See `elgrep' and `elgrep-menu' for details.

(fn)" t)
(autoload 'elgrep-menu-arg-list "elgrep" "\
Collect `elgrep' arguments from `elgrep-menu' buffer." t)
...
(fn DIR FILE-NAME-RE RE &rest OPTIONS)" t)
(autoload 'elgrep-occur "elgrep" "\
Run elgrep `occur'-like for REGEXP on the current buffer.
OPTIONS are the same as for the command `elgrep'.

(fn REGEXP &rest OPTIONS)" t)
(autoload 'elgrep-search "elgrep" "\
In path DIR grep files with name matching FILE-NAME-RE for text matching RE.
This is done via Emacs Lisp (no dependence on external grep).
Return list of filematches.
...
(autoload 'elgrep-load-elgrep-data-file "elgrep" "\
Load the `elgrep-data-file'." t)
(autoload 'elgrep-save-elgrep-data-file "elgrep" "\
Save the elgrep data file if `elgrep-data-file' is a string.
This can be used as `kill-emacs-hook'.
Unconditionally return the value of `elgrep-data-file'." t)
(register-definition-prefixes "elgrep" '("elgrep"))

;;; End of scraped data

(provide 'elgrep-autoloads)
