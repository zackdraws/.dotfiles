...
for file in glob.glob('*.jpg') + glob.glob('*.jpeg'):
    ...
    # -------- Clean transparent line drawing --------

    gray_line = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)



    # Softly remove white background

    _, alpha_line = cv2.threshold(gray_line, 245, 255, cv2.THRESH_BINARY_INV)

    alpha_line = cv2.GaussianBlur(alpha_line, (3, 3), 0)



    # Convert grayscale to RGB and merge with alpha
    ...
    # Optional: gently brighten lines if needed
    ...
    print(f"✅ Saved matte, composite, and line art for {file}")
...
echo "✅ All done! Files saved in: 'matte/', 'composite/', and 'line/' folders."
